...
| FR-08 | **Explanation [decided, EDN-09]:** `/predict` returns the base price and the five features with the largest absolute SHAP contribution, plus the combined contribution of all other features, computed with `shap.TreeExplainer` (Python 3.12). The model predicts `log(price)`, so a contribution φ is reported as its multiplicative effect on the price, `exp(φ) - 1` in percent, and the effects multiply rather than add. | API test: the base price times the product of `1 + effect` over all six reported effects reproduces the estimate |
